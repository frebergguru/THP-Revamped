...
//if $action is "CMoveUp" then
if ($action=="CMoveUp") {
    move_box($mlink, $id, 'up', $id2, 'c');
//else if $action is "CMoveDown" then
} elseif ($action=="CMoveDown") {
    move_box($mlink, $id, 'down', $id2, 'c');
//if $action is "CAdded" then
} elseif ($action=="CAdded") {
	//add the text to the selected page
    // Get the highest sort_id for this position and catid
    $res = mysqli_query($mlink, "SELECT MAX(sort_id) as max_sort FROM boxes WHERE catid='$id2' AND position='c'");
    $row = mysqli_fetch_array($res);
    $new_sort_id = $row['max_sort'] + 1;
	mysqli_query($mlink, "INSERT INTO `boxes` ( `site` , `catid` , `position` , `headline` , `uheadline` , `link` , `text`, `image`, `module`, `sort_id`, `allsites` ) VALUES ('$id2', '$id2', 'c', '$editheadline', '$edituheadline', '$editlink', '$edittext', '', '', '$new_sort_id', '$editallsites')") or mysqldie("Kan ikke skrive til $database.boxes");
//else if $action is like "CAddedI" then
	...
	// Check if an existing image is selected
		...
		if (file_exists($imagefile)) {
            ...
            $row = mysqli_fetch_array($res);
            $new_sort_id = $row['max_sort'] + 1;
			$query_string = "INSERT INTO `boxes` ( `site` , `catid` , `position` , `headline` , `image`, `uheadline`, `link`, `text`, `module`, `sort_id`, `allsites` ) VALUES ('$id2', '$id2', 'c', '$editheadline', '$editimage', '', '', '', '', '$new_sort_id', '$editallsites')";
...
//Query the database table boxes where catid is $id2 and position is c order by sort_id descending
$result=mysqli_query($mlink, "SELECT * FROM boxes WHERE (catid='$id2' OR allsites='1') AND position='c' ORDER BY sort_id DESC") or mysqldie("Kan ikke lese fra $database.boxes");
...
print '<div class="flex-table">
    <div class="flex-content">
        <strong><a href="?site='.$site.'&amp;id2='.$id2.'&amp;action=CMoveUp&amp;id='.$id.'">Opp</a> || <a href="?site='.$site.'&amp;id2='.$id2.'&amp;action=CMoveDown&amp;id='.$id.'">Ned</a> || <a href="?site='.$site.'&amp;id2='.$id2.'&amp;action=';if (!empty($image)) {print 'CEditI';}else{print 'CEdit';};print '&amp;id='.$id.'">Rediger</a> || <a href="?site='.$site.'&amp;id2='.$id2.'&amp;action=';if(!empty($image)){print 'CDeleteI';}else{print 'CDelete';};print '&amp;id='.$id.'">Slett</a></strong>
    </div>
    <div class="flex-header">
