...
$result=mysqli_query($mlink, "SELECT * FROM guestbook WHERE message LIKE '%".mysqli_real_escape_string($mlink, $search)."%' OR name LIKE '%".mysqli_real_escape_string($mlink, $search)."%' OR ip LIKE '%".mysqli_real_escape_string($mlink, $search)."%' ORDER BY id") or mysqldie ("Kan ikke lese fra $database.guestbook");
...
print '<div class="flex-table">
    <div class="flex-header">
        ...
        <strong>Fra:</strong> '.$name.' <strong>Dato:</strong> '.$date.'
        </font>
    </div>
    <div class="flex-content">
        <font size="2">
